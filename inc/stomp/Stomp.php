...
class StompConnection {
    ...
    function StompConnection($brokerUri) {
        $ereg = "^(([a-zA-Z]+)://)+\(*([a-zA-Z0-9\.:/,]+)\)*\??([a-zA-Z0-9=]*)$";
    ...
    function processUrl($url) {
       ...
       if ($parsed) {
         $host = $parsed['host'];
         $port = $parsed['port'];
         array_push($this->hosts, array($parsed['host'], $parsed['port']));
       } else {
    ...
    function makeConnection() {
      ...
      while (!$connected && $att++ < $this->attempts) {
        if (isset($this->params['randomize'])
            && $this->params['randomize'] != null 
        ...
        $host = $broker[0];
        $port = $broker[1];
        // trigger_error("connecting to: $host:$port");
        if ($port == null) {
        ...
        if ($this->socket == null) {
          trigger_error("Could not connect to $host:$port", E_USER_WARNING);
        } else {
